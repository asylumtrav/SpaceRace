...
#      ◦ Moved **after** `unlocks` is defined so that `unlocks` actually exists.
# -------------------------------------------------------------------------------
...
if loaded is None:
    ...
    money                       = game_state.get("money", 50000000000000.0)
...
STARTING_MONEY = 50000000000000.0  # Keep consistent across resets
    ...
    for idx, u in enumerate(unlocks):
        ...
        if u["biz_index"] is not None:
            ...
            if all(b["owned"] >= u["threshold"] for b in businesses):
                ...
                popup_message = {
                    "text": u["description"],
                    "requirement": f"Own {u['threshold']} of every business"
                }
                popup_end_time = now + 3000
