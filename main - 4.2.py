...
def default_game_state():
    ...
    return {
        "money": 5.0,
        "space_lifetime_earnings": 100000000000000000000000000000000000000,
...
if loaded is None:
    ...
    space_lifetime_earnings     = game_state.get("space_lifetime_earnings", 100000000000000000000000000000000000000)
...
def draw_investors_ui(surface, mouse_pos, mouse_clicked):
    ...
    global show_investor_shop, galactic_investors_total, galactic_investors_spent
    global investor_shop_scroll, global_speed_mult, global_profit_mult
    global money, space_lifetime_earnings
    global cycle_start_time, cycle_start_money, playtime_this_prestige
    global prestige_count
    ...
    if not show_investor_shop:
        ...
        if invest_btn_rect.collidepoint(mouse_pos) and mouse_clicked and new_gis > 0:
            # ── 1) Collect all available GIs ──
            lifetime_q     = space_lifetime_earnings / 1e15
            potential_amt  = int(150 * math.sqrt(lifetime_q)) if lifetime_q > 0 else 0
            collect_amt    = max(0, potential_amt - galactic_investors_spent)
            galactic_investors_total += collect_amt
            galactic_investors_spent += collect_amt

            # ── 2) Increment prestige counter ──
            prestige_count += 1

            # ── 3) Reset “normal” progress (money, businesses, unlocks, upgrades) ──
            # If this is the 2nd (or later) prestige, give a free Asteroid Miner instead of $5
            if prestige_count > 1:
                money = 0.0
            else:
                money = 5.0
            ...
            # Always unlock Asteroid Miner (index 0)
            businesses[0]["unlocked"] = True

            # If this is >= second prestige, also give one free Asteroid Miner
            if prestige_count > 1:
        ...
        # ── Investor Shop sub‐view (… unchanged …) ──
        title_surf = font_big.render("GALACTIC INVESTOR SHOP", True, WHITE)
        surface.blit(title_surf, (box_x + (box_w - title_surf.get_width()) // 2, box_y + 20))

        # Replace “back arrow” with a simple “X” in top right:
        back_x = box_x + box_w - close_size - 12
        back_y = box_y + 12
        ...
        arrow_surf = font_small.render("X", True, WHITE)
        surface.blit(
            arrow_surf,
...
def draw_investor_shop_list(surface, mouse_pos, mouse_clicked):
    ...
    for upgrade in unpurchased:
        ...
        # ───── ICON ─────
        try:
            img = pygame.image.load(upgrade["icon_image"]).convert_alpha()
