...
# -----------------------------------------------------------------------------
# 2. SAVE & LOAD FUNCTIONS (save to ./saves/save.json)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 3. UI UTILITIES (including format_number_parts) – MUST BE DEFINED BEFORE USAGE
# -----------------------------------------------------------------------------
def format_number_parts(n):
    """
    Split an integer into a mantissa and suffix (e.g., 1,234,000 → ("1.234", "M")).
    """
...
# -----------------------------------------------------------------------------
# 4. OFFLINE EARNINGS CALCULATION (fixed: all owned businesses run offline)
# -----------------------------------------------------------------------------
def calculate_offline_earnings(data_loaded: dict, now_ts: float):
    ...
    for biz in data_loaded["businesses"]:
        if biz["owned"] <= 0:
            continue
        ...
        gi_bonus     = 1.0 + (0.02 * galactic_investors_total)
        cycle_value  = base_payout * owned * profit_mult * global_profit_mult * gi_bonus

        earned = cycle_value * cycles
...
# To change icon size, adjust the tuple below (e.g., (64, 64) or (128, 128))
icon_path = resource_path("assets/starlightfarm.png")
if os.path.isfile(icon_path):
    try:
        raw_icon = pygame.image.load(icon_path).convert_alpha()
        icon_surf = pygame.transform.smoothscale(raw_icon, (96, 96))  # <--- ICON SIZE DEFINED HERE
...
# -----------------------------------------------------------------------------
# 7. BUSINESS DEFINITIONS (3× scaled, with asset_path)
# -----------------------------------------------------------------------------
businesses = [
    {
        "index": 0,
...
# -----------------------------------------------------------------------------
# 8. LOAD ALL BUSINESS IMAGES (scale to 96×96)
# -----------------------------------------------------------------------------
        ...
        if rel:
            ...
            if os.path.isfile(abs_p):
                try:
                    ...
                    biz["image"] = pygame.transform.smoothscale(img, (96, 96))  # <--- SCALE SIZE HERE
                except Exception:
                    biz["image"] = None
            else:
                biz["image"] = None
...
# -----------------------------------------------------------------------------
# 10. UNLOCKS (Business-Specific + Global)
# -----------------------------------------------------------------------------
unlocks = [
    {"biz_index": 0, "threshold": 10,  "type": "speed",       "multiplier": 1.5,
     "description": "Asteroid Miner speed ×1.5 (Own 10)"},
    {"biz_index": 0, "threshold": 25,  "type": "speed",       "multiplier": 2.0,
     "description": "Asteroid Miner speed ×2 (Own 25)"},
    {"biz_index": 0, "threshold": 50,  "type": "speed",       "multiplier": 3.0,
     "description": "Asteroid Miner speed ×3 (Own 50)"},
    {"biz_index": 0, "threshold": 100, "type": "profit",      "multiplier": 2.0,
     "description": "Asteroid Miner profit ×2 (Own 100)"},
    {"biz_index": 0, "threshold": 200, "type": "speed",       "multiplier": 4.0,
     "description": "Asteroid Miner speed ×4 (Own 200)"},
    {"biz_index": 0, "threshold": 500, "type": "profit",      "multiplier": 3.0,
     "description": "Asteroid Miner profit ×3 (Own 500)"},

    {"biz_index": 1, "threshold": 10,  "type": "speed",       "multiplier": 1.5,
     "description": "Satellite Network speed ×1.5 (Own 10)"},
    {"biz_index": 1, "threshold": 25,  "type": "speed",       "multiplier": 2.0,
     "description": "Satellite Network speed ×2 (Own 25)"},
    {"biz_index": 1, "threshold": 50,  "type": "speed",       "multiplier": 3.0,
     "description": "Satellite Network speed ×3 (Own 50)"},
    {"biz_index": 1, "threshold": 100, "type": "profit",      "multiplier": 2.0,
     "description": "Satellite Network profit ×2 (Own 100)"},
    {"biz_index": 1, "threshold": 200, "type": "speed",       "multiplier": 4.0,
     "description": "Satellite Network speed ×4 (Own 200)"},
    {"biz_index": 1, "threshold": 500, "type": "profit",      "multiplier": 3.0,
     "description": "Satellite Network profit ×3 (Own 500)"},

    {"biz_index": 2, "threshold": 10,  "type": "speed",       "multiplier": 1.5,
     "description": "Rocket Yard speed ×1.5 (Own 10)"},
    {"biz_index": 2, "threshold": 25,  "type": "speed",       "multiplier": 2.0,
     "description": "Rocket Yard speed ×2 (Own 25)"},
    {"biz_index": 2, "threshold": 50,  "type": "speed",       "multiplier": 3.0,
     "description": "Rocket Yard speed ×3 (Own 50)"},
    {"biz_index": 2, "threshold": 100, "type": "profit",      "multiplier": 2.0,
     "description": "Rocket Yard profit ×2 (Own 100)"},
    {"biz_index": 2, "threshold": 200, "type": "speed",       "multiplier": 4.0,
     "description": "Rocket Yard speed ×4 (Own 200)"},
    {"biz_index": 2, "threshold": 500, "type": "profit",      "multiplier": 3.0,
     "description": "Rocket Yard profit ×3 (Own 500)"},

    {"biz_index": 3, "threshold": 10,  "type": "speed",       "multiplier": 1.5,
     "description": "Lunar Colony speed ×1.5 (Own 10)"},
    {"biz_index": 3, "threshold": 25,  "type": "speed",       "multiplier": 2.0,
     "description": "Lunar Colony speed ×2 (Own 25)"},
    {"biz_index": 3, "threshold": 50,  "type": "speed",       "multiplier": 3.0,
     "description": "Lunar Colony speed ×3 (Own 50)"},
    {"biz_index": 3, "threshold": 100, "type": "profit",      "multiplier": 2.0,
     "description": "Lunar Colony profit ×2 (Own 100)"},
    {"biz_index": 3, "threshold": 200, "type": "speed",       "multiplier": 4.0,
     "description": "Lunar Colony speed ×4 (Own 200)"},
    {"biz_index": 3, "threshold": 500, "type": "profit",      "multiplier": 3.0,
     "description": "Lunar Colony profit ×3 (Own 500)"},

    {"biz_index": 4, "threshold": 10,  "type": "speed",       "multiplier": 1.5,
     "description": "Starlight Farm speed ×1.5 (Own 10)"},
    {"biz_index": 4, "threshold": 25,  "type": "speed",       "multiplier": 2.0,
     "description": "Starlight Farm speed ×2 (Own 25)"},
    {"biz_index": 4, "threshold": 50,  "type": "speed",       "multiplier": 3.0,
     "description": "Starlight Farm speed ×3 (Own 50)"},
    {"biz_index": 4, "threshold": 100, "type": "profit",      "multiplier": 2.0,
     "description": "Starlight Farm profit ×2 (Own 100)"},
    {"biz_index": 4, "threshold": 200, "type": "speed",       "multiplier": 4.0,
     "description": "Starlight Farm speed ×4 (Own 200)"},
    {"biz_index": 4, "threshold": 500, "type": "profit",      "multiplier": 3.0,
     "description": "Starlight Farm profit ×3 (Own 500)"},

    {"biz_index": 5, "threshold": 10,  "type": "speed",       "multiplier": 1.5,
     "description": "Alien Outpost speed ×1.5 (Own 10)"},
    {"biz_index": 5, "threshold": 25,  "type": "speed",       "multiplier": 2.0,
     "description": "Alien Outpost speed ×2 (Own 25)"},
    {"biz_index": 5, "threshold": 50,  "type": "speed",       "multiplier": 3.0,
     "description": "Alien Outpost speed ×3 (Own 50)"},
    {"biz_index": 5, "threshold": 100, "type": "profit",      "multiplier": 2.0,
     "description": "Alien Outpost profit ×2 (Own 100)"},
    {"biz_index": 5, "threshold": 200, "type": "speed",       "multiplier": 4.0,
     "description": "Alien Outpost speed ×4 (Own 200)"},
    {"biz_index": 5, "threshold": 500, "type": "profit",      "multiplier": 3.0,
     "description": "Alien Outpost profit ×3 (Own 500)"},

    {"biz_index": 6, "threshold": 10,  "type": "speed",       "multiplier": 1.5,
     "description": "Solar Array speed ×1.5 (Own 10)"},
    {"biz_index": 6, "threshold": 25,  "type": "speed",       "multiplier": 2.0,
     "description": "Solar Array speed ×2 (Own 25)"},
    {"biz_index": 6, "threshold": 50,  "type": "speed",       "multiplier": 3.0,
     "description": "Solar Array speed ×3 (Own 50)"},
    {"biz_index": 6, "threshold": 100, "type": "profit",      "multiplier": 2.0,
     "description": "Solar Array profit ×2 (Own 100)"},
    {"biz_index": 6, "threshold": 200, "type": "speed",       "multiplier": 4.0,
     "description": "Solar Array speed ×4 (Own 200)"},
    {"biz_index": 6, "threshold": 500, "type": "profit",      "multiplier": 3.0,
     "description": "Solar Array profit ×3 (Own 500)"},

    {"biz_index": 7, "threshold": 10,  "type": "speed",       "multiplier": 1.5,
     "description": "Black Hole Labs speed ×1.5 (Own 10)"},
    {"biz_index": 7, "threshold": 25,  "type": "speed",       "multiplier": 2.0,
     "description": "Black Hole Labs speed ×2 (Own 25)"},
    {"biz_index": 7, "threshold": 50,  "type": "speed",       "multiplier": 3.0,
     "description": "Black Hole Labs speed ×3 (Own 50)"},
    {"biz_index": 7, "threshold": 100, "type": "profit",      "multiplier": 2.0,
     "description": "Black Hole Labs profit ×2 (Own 100)"},
    {"biz_index": 7, "threshold": 200, "type": "speed",       "multiplier": 4.0,
     "description": "Black Hole Labs speed ×4 (Own 200)"},
    {"biz_index": 7, "threshold": 500, "type": "profit",      "multiplier": 3.0,
     "description": "Black Hole Labs profit ×3 (Own 500)"},

    {"biz_index": 8, "threshold": 10,  "type": "speed",       "multiplier": 1.5,
     "description": "Wormhole Gate speed ×1.5 (Own 10)"},
    {"biz_index": 8, "threshold": 25,  "type": "speed",       "multiplier": 2.0,
     "description": "Wormhole Gate speed ×2 (Own 25)"},
    {"biz_index": 8, "threshold": 50,  "type": "speed",       "multiplier": 3.0,
     "description": "Wormhole Gate speed ×3 (Own 50)"},
    {"biz_index": 8, "threshold": 100, "type": "profit",      "multiplier": 2.0,
     "description": "Wormhole Gate profit ×2 (Own 100)"},
    {"biz_index": 8, "threshold": 200, "type": "speed",       "multiplier": 4.0,
     "description": "Wormhole Gate speed ×4 (Own 200)"},
    {"biz_index": 8, "threshold": 500, "type": "profit",      "multiplier": 3.0,
     "description": "Wormhole Gate profit ×3 (Own 500)"},

    {"biz_index": 9, "threshold": 10,  "type": "speed",       "multiplier": 1.5,
     "description": "Galactic Senate speed ×1.5 (Own 10)"},
    {"biz_index": 9, "threshold": 25,  "type": "speed",       "multiplier": 2.0,
     "description": "Galactic Senate speed ×2 (Own 25)"},
    {"biz_index": 9, "threshold": 50,  "type": "speed",       "multiplier": 3.0,
     "description": "Galactic Senate speed ×3 (Own 50)"},
    {"biz_index": 9, "threshold": 100, "type": "profit",      "multiplier": 2.0,
     "description": "Galactic Senate profit ×2 (Own 100)"},
    {"biz_index": 9, "threshold": 200, "type": "speed",       "multiplier": 4.0,
     "description": "Galactic Senate speed ×4 (Own 200)"},
    {"biz_index": 9, "threshold": 500, "type": "profit",      "multiplier": 3.0,
     "description": "Galactic Senate profit ×3 (Own 500)"},

    {"biz_index": None, "threshold": 50,  "type": "global_speed", "multiplier": 1.5,
     "description": "Global speed ×1.5 (Own 50 of everything)"},
    {"biz_index": None, "threshold": 100, "type": "global_both",  "multiplier": 2.0,
     "description": "Global speed ×2 & profit ×2 (Own 100 of everything)"},
    {"biz_index": None, "threshold": 500, "type": "global_both",  "multiplier": 3.0,
     "description": "Global speed ×3 & profit ×3 (Own 500 of everything)"},
]

# -----------------------------------------------------------------------------
# 11. GALACTIC UPGRADES
# -----------------------------------------------------------------------------
galactic_upgrades = [
    {
...
if loaded is None:
    ...
    # Restore all per-business fields
    for idx, biz_saved in enumerate(game_state["businesses"]):
        businesses[idx]["owned"]       = biz_saved["owned"]
        businesses[idx]["speed_mult"]  = biz_saved["speed_mult"]
        businesses[idx]["profit_mult"] = biz_saved["profit_mult"]
        businesses[idx]["unlocked"]    = biz_saved["unlocked"]
        businesses[idx]["has_manager"] = biz_saved["has_manager"]

    # Restore all upgrades
    for i, upg_saved in enumerate(game_state["upgrades"]):
        upgrades[i]["purchased"] = upg_saved["purchased"]

    unlocked_shown = set(game_state.get("unlocked_shown", []))

    for i, gu_saved in enumerate(game_state.get("galactic_upgrades", [])):
        galactic_upgrades[i]["purchased"] = gu_saved["purchased"]

    # Restore core numeric values
    money                       = game_state.get("money", 5000000000000000)
    ...
    # === Calculate offline earnings for all owned businesses ===
    now_ts = time.time()
    offline_earned = calculate_offline_earnings(game_state, now_ts)
    if offline_earned > 0:
        # Immediately reflect that change in the local `money` variable:
        money = game_state["money"]
        ...
        # Show popup for 3 seconds using pygame.get_ticks()
        popup_end_time = pygame.time.get_ticks() + 3000

    # Start any manager-run businesses back into "in progress" state
    for idx, biz_saved in enumerate(game_state["businesses"]):
        if businesses[idx]["has_manager"] and businesses[idx]["owned"] > 0:
            businesses[idx]["in_progress"] = True
            effective_time = (businesses[idx]["base_time"] / businesses[idx]["speed_mult"]) / global_speed_mult
            businesses[idx]["timer"] = effective_time

    # Reload images so biz["image"] is never None
    load_all_business_images()
...
def draw_business_panel(surface, dt, mouse_pos, mouse_clicked):
    ...
    for idx, biz in enumerate(businesses):
        ...
        # --- UPDATED ICON POSITION & SIZE HERE ---
        if biz.get("image"):
            icon_surf = biz["image"]
            # icon is scaled to 96×96 by load_all_business_images()
            icon_x = x - 2    # SHIFT LEFT (adjust as needed)
            icon_y = y + (ROW_HEIGHT - 96) // 2  # CENTER VERTICALLY for a 96px icon
            if not unlocked:
            ...
            surface.blit(icon_surf, (x + 22 + 16, y + 22 + 16))
        # --- END ICON BLOCK ---
        ...
        # HITBOX: match 96×96 icon
        if biz.get("image"):
            icon_hitbox = pygame.Rect(x - 2, y + (ROW_HEIGHT - 96) // 2, 96, 96)
        else:
            icon_hitbox = pygame.Rect(x + 22, y + 22, 64, 64)
        ...
        if biz["in_progress"]:
            ...
            if effective_time <= stripe_threshold:
                ...
                stripe_speed = bar_w / effective_time if effective_time > 0 else bar_w
                offset = int((pygame.time.get_ticks() / 1000.0) * stripe_speed) % (20 + 20)
                stripe_w = 20
...
def draw_managers_ui(surface, mouse_pos, mouse_clicked):
    global close_btn_rect, money, manager_scroll
...
def draw_upgrades_ui(surface, mouse_pos, mouse_clicked):
    global close_btn_rect, money, upgrade_scroll
    ...
    for upg in upgrades:
        ...
        if buy_rect.collidepoint(mouse_pos) and mouse_clicked and can_buy:
            money -= upg["cost"]
            businesses[upg["biz_index"]]["profit_mult"] *= upg["multiplier"]
            upg["purchased"] = True
...
def draw_unlocks_ui(surface, mouse_pos, mouse_clicked):
    global close_btn_rect, unlock_scroll
    ...
    for idx, u in enumerate(unlocks):
        ...
        if u["biz_index"] is not None:
            ...
            req_text = f"Own {u['threshold']} of everything"
...
def draw_investor_shop_list(surface, mouse_pos, mouse_clicked):
    ...
    for upgrade in galactic_upgrades:
        ...
        if buy_rect.collidepoint(mouse_pos) and mouse_clicked and can_buy:
            ...
            apply_galactic_upgrade_effect(upgrade)
...
def draw_popup(surface):
    ...
    popup_bg.fill((40, 42, 56, 220))
    pygame.draw.rect(popup_bg, PANEL_DARK, (0, 0, pw, ph), border_radius=8)
    surface.blit(popup_bg, (px, py))

    text = popup_message["text"]
    req  = popup_message["requirement"]

    line1 = font_med.render(text, True, WHITE)
    line2 = font_small.render(req, True, GRAYED)

    surface.blit(line1, (px + 12, py + 12))
    surface.blit(line2, (px + 12, py + 44))

# -----------------------------------------------------------------------------
# 15. MAIN LOOP (SAVE ON EXIT)
# -----------------------------------------------------------------------------
running = True
mouse_down    = False
mouse_clicked = False

def resize_ui(new_w, new_h):
    global WIDTH, HEIGHT
    global font_big, font_med, font_small
    global SIDEBAR_WIDTH, HEADER_HEIGHT, PANEL_X, PANEL_Y, PANEL_WIDTH, PANEL_HEIGHT
    ...
    base_size = max(16, int(24 * (HEIGHT / 720)))
    font_big   = pygame.font.SysFont(None, base_size + 12)
    font_med   = pygame.font.SysFont(None, base_size)
    font_small = pygame.font.SysFont(None, max(14, int(base_size * 0.75)))
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            game_state["space_lifetime_earnings"]   = space_lifetime_earnings
            game_state["global_speed_mult"]         = global_speed_mult
            game_state["global_profit_mult"]        = global_profit_mult
            game_state["galactic_investors_total"]  = galactic_investors_total
            game_state["galactic_investors_spent"]  = galactic_investors_spent
            ...
            game_state["last_timestamp"] = time.time()
            save_game(game_state)
            running = False

        elif event.type == pygame.VIDEORESIZE:
            resize_ui(event.w, event.h)
            ...
            if event.button == 1:
                ...
                if overlay_mode == "Managers":
                    ...
                    if show_investor_shop:
                        investor_shop_scroll -= 40
                elif (PANEL_X <= mouse_pos[0] <= PANEL_X + PANEL_WIDTH and
    ...
    if sidebar_click is not None and mouse_clicked:
        overlay_mode = ["Managers", "Upgrades", "Unlocks", "Investors"][sidebar_click]
        if overlay_mode != last_overlay_mode:
            if overlay_mode == "Managers":
                manager_scroll = 0
            elif overlay_mode == "Upgrades":
                upgrade_scroll = 0
                ...
                show_investor_shop = False
                investor_shop_scroll = 0
        last_overlay_mode = overlay_mode
    ...
    for idx, u in enumerate(unlocks):
        ...
        if u["biz_index"] is not None:
            ...
            if biz["owned"] >= u["threshold"]:
                ...
                popup_message = {
                    "text": u["description"],
                    "requirement": f"Own {u['threshold']} {biz['name']}"
                }
                popup_end_time = now + 3000
        else:
            if all(b["owned"] >= u["threshold"] for b in businesses):
                unlocked_shown.add(idx)
                if u["type"] == "global_speed":
                    global_speed_mult *= u["multiplier"]
                elif u["type"] == "global_both":
                    global_speed_mult  *= u["multiplier"]
                    global_profit_mult *= u["multiplier"]
                popup_message = {
                    "text": u["description"],
                    "requirement": f"Own {u['threshold']} of everything"
                }
                popup_end_time = now + 3000

    if overlay_mode == "Managers":
        cb = draw_managers_ui(screen, mouse_pos, mouse_clicked)
        if mouse_clicked and cb and cb.collidepoint(mouse_pos):
            overlay_mode = None

    elif overlay_mode == "Upgrades":
        cb = draw_upgrades_ui(screen, mouse_pos, mouse_clicked)
        if mouse_clicked and cb and cb.collidepoint(mouse_pos):
            overlay_mode = None

    elif overlay_mode == "Unlocks":
        cb = draw_unlocks_ui(screen, mouse_pos, mouse_clicked)
        if mouse_clicked and cb and cb.collidepoint(mouse_pos):
            overlay_mode = None

    elif overlay_mode == "Investors":
        cb = draw_investors_ui(screen, mouse_pos, mouse_clicked)
        if mouse_clicked:
            if not show_investor_shop and cb and cb.collidepoint(mouse_pos):
                overlay_mode = None
            elif show_investor_shop and cb and cb.collidepoint(mouse_pos):
                show_investor_shop = False
