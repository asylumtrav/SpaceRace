...
def load_all_business_images():
    ...
    # Also assign the same global icon to cash-upgrades with biz_index=None
    if 'gimg' in locals():
        for upg in upgrades:
...
def draw_business_panel(surface, dt, mouse_pos, mouse_clicked):
    ...
    for idx, biz in enumerate(businesses):
        ...
        if biz["in_progress"]:
            # Compute how much time this cycle should take
            effective_time = (biz["base_time"] / biz["speed_mult"]) / global_speed_mult

            # Decrement timer by dt and check for completion
            biz["timer"] -= dt
            if biz["timer"] <= 0:
                biz["timer"] = 0.0
                biz["in_progress"] = False
            ...
            # Calculate fraction completed (0.0 → 1.0)
            pct = max(0.0, min(1.0, 1.0 - (biz["timer"] / effective_time)))
            fill_w = int(bar_w * pct)

            if fill_w > 0:
                # 1) Draw the light-green fill up to fill_w
                pygame.draw.rect(surface, PROGRESS_FILL, (bar_x, bar_y, fill_w, bar_h), border_radius=7)

                # 2) Draw dark-green stripes over that filled region
                clip_rect = pygame.Rect(bar_x, bar_y, fill_w, bar_h)
                ...
                while x_pos < bar_x + fill_w:
                    surface.blit(stripe_surf, (x_pos, bar_y))
                    x_pos += stripe_w + 20

                # Restore clipping so nothing else gets clipped
                surface.set_clip(None)

            else:
                # Solid fill (no stripes)
                pygame.draw.rect(surface, PROGRESS_FILL, (bar_x, bar_y, fill_w, bar_h), border_radius=7)
